...
# ─── Fixed constants (match original preprocess_chbmit.py exactly) ─────
DATA_DIR      = r'D:\chbmit_data'
TEMP_DIR_ROOT = r'D:\chbmit_temp'
...
PRE_ICTAL_STEP   = 10 * FS
INTER_ICTAL_STEP = 5 * 60 * FS
...
def make_labeller(pre_min_s, pre_max_s, post_gap_s):
    """Build get_window_label and find_discard_end_s closures for given params."""
    def get_window_label(abs_start_s, abs_end_s, all_seizures_abs):
        is_preictal = False
        for sz_start, sz_end in all_seizures_abs:
            too_close = sz_start - pre_min_s
            pre_start = sz_start - pre_max_s
            post_end  = sz_end + post_gap_s
    ...
    return get_window_label, find_discard_end_s
...
def process_patient(p_id, out_dir, temp_dir, get_window_label, find_discard_end_s):
...
def main():
    ap = argparse.ArgumentParser(
        description='Preprocess CHB-MIT with configurable postictal gap '
                    'and preictal horizon, for the sensitivity ablation.')
    ap.add_argument('--postictal-gap-h', type=float, default=4.0,
                    help='Postictal exclusion gap in hours (default 4; '
                         'use 0, 1, or 4 for the sensitivity sweep).')
    ap.add_argument('--preictal-min', type=float, default=5.0,
                    help='Preictal window starts this many minutes before onset '
                         '(default 5 - matches original).')
    ap.add_argument('--preictal-max', type=float, default=30.0,
                    help='Preictal window extends back this many minutes before '
                         'onset (default 30 - matches original).')
    ap.add_argument('--out-dir', type=str, required=True,
                    help='Output directory for preprocessed *_X.npy / *_y.npy.')
    args = ap.parse_args()

    pre_min_s  = args.preictal_min * 60.0
    pre_max_s  = args.preictal_max * 60.0
    post_gap_s = args.postictal_gap_h * 3600.0

    print(f'Preprocessing with: postictal_gap={args.postictal_gap_h} h, '
          f'preictal={args.preictal_min}-{args.preictal_max} min, '
          f'out_dir={args.out_dir}')

    get_window_label, find_discard_end_s = make_labeller(
        pre_min_s, pre_max_s, post_gap_s)

    out_dir  = args.out_dir
    # sanitise temp dir name per-config so concurrent runs don't collide
    tag      = f'gap{int(args.postictal_gap_h*10):03d}_pre{int(args.preictal_min)}-{int(args.preictal_max)}'
    temp_dir = os.path.join(TEMP_DIR_ROOT, tag)
    ...
    if os.path.exists(DATA_DIR):
        ...
        for p in patients:
            process_patient(p, out_dir, temp_dir,
                            get_window_label, find_discard_end_s)
        print("\n" + "=" * 40 + "\nAll done!\n" + "=" * 40)
    else:
