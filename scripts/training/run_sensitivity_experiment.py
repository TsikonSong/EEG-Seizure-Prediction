...
# ══════════════════════════════════════════════════════════════════════
#  TCN architecture  (matches your train_tcn.ipynb exactly)
# ══════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════
#  Architecture registry - per-arch hyperparameters
# ══════════════════════════════════════════════════════════════════════
ARCHITECTURES = {
    'tcn': {
        'class':      TCN,
...
# ══════════════════════════════════════════════════════════════════════
#  Main loop - 2 architectures × 3 configs × 5 seeds = 30 runs
# ══════════════════════════════════════════════════════════════════════
def main():
    t_global = time.time()
    ...
    def _check(arch, cfg_name, target_mean, target_sd):
        ...
        if not results:
            print(f'  {arch} {cfg_name}: results missing'); return
        aucs = np.array([r['test_auc'] for r in results])
        mean = aucs.mean()
    ...
    _check('tcn',    'gap_4h', 0.574, 0.059)
    _check('eegnet', 'gap_4h', 0.564, 0.072)

    print('\nIf both rows say OK, these JSON files are ready for review:')
    for arch in ARCHITECTURES:
        for cfg_name in PREPROCESS_CONFIGS:
            path = os.path.join(r'D:\seizure_results',
                                f'{arch}_{cfg_name}', 'results.json')
            print(f'  {path}')
    print('\nIf either row is off by more than 1sd, do not trust 0h/1h numbers;')
